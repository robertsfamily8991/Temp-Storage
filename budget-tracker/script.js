...
function occursOnDate(entry, day) {
  ...
  switch (entry.recurrence) {
    case "once":
      return (
        entry.date === buildISODateLocal(day.getFullYear(), day.getMonth(), day.getDate())
      );
    case "weekly":
...
function renderCalendar() {
  ...
  logMonth.textContent = `${currentMonth.toLocaleString("default", {
    month: "long",
  })} ${y}`;
...
function renderLog() {
  ...
  switch (sortLog.value) {
    ...
    case "title":
      ...
      break;
    case "rule":
      sorted.sort((a, b) => a.recurrence.localeCompare(b.recurrence));
      break;
  }
  ...
  sorted.forEach((e, i) => {
    ...
    logBody.innerHTML += `
      <tr class="${e.type}">
        ...
        <td>${ruleDisp}</td>
        <td>
          <button class="btn-secondary" onclick="openModalForEntry(${entries.indexOf(e)},event)">Edit</button>
          <button class="btn-danger" onclick="modalDeleteHandler(${entries.indexOf(e)})">Delete</button>
        </td>
      </tr>`;
  });
}
  ...
  for (let d = 1; d <= lastDay; d++) {
    ...
    entries.forEach((e) => {
      if (occursOnDate(e, day)) {
        ...
        else expenses += Number(e.amount) || 0; // expense & investment counted
      }
    });
  }
...
openAddEntryBtn.addEventListener("click", () =>
  openModalForDate(
    buildISODateLocal(currentMonth.getFullYear(), currentMonth.getMonth(), new Date().getDate())
  )
);
